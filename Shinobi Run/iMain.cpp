...
void collision()
{
	if (level_1){
		if (pause){
			...
			{
				if (p_b_x + 37 >= boss_pow_x && p_b_x + 37 <= boss_pow_x + 100 && p_b_y >= boss_pow_y && p_b_y <= boss_pow_y + 100)
				{
					bossPowerShow = false;
					p_bS = false;
					p_b_x = k_x + 10000000000000000000;
...
void health_coll()
{
	if (level_1){
		...
		if (pause){
			...
			for (int i = 0; i < BOSSPOWER2; i++)
			{
				if (k_x + 100 >= boss2_pow_x && k_x <= boss2_pow_x + 160 && k_y + jCor >= boss2_pow_y && k_y + jCor <= boss2_pow_y + 150)
				{
					if (healthIndex <= 3){
						if (boss2PowerShow){
							int a = 20;
							healthIndex++;
							boss2PowerShow = false;
							boss2_pow_x = a * -1;
						}
						cout << healthIndex << endl;
					}
